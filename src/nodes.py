...
from src.states import DietState, SearchState, IntolerancesState, ForbiddenFoodsState
from duckduckgo_search import DDGS 
import json
import kagglehub
from src.utils import retrieve_product_info
...
prompts = load_prompt("prompts.json")

def assistant_dietician(state: DietState) -> str:
    """
    This node is an assitant dietician that will help the user with their diet.
    ...
    - Intorlerance_search: to search in DuckDuckGo for which foods the user is intolerant to.
    - Diet_search: search the best diets for the user.
    - Buget_search: search for the best recipes that fit the user's budget with the db of the supermarket.
...
def intolerance_search(state: DietState) -> DietState:
    ...
    with DDGS() as ddgs:
        for intolerance in new_intolerances['intolerances']:
            ...
            for result in ddgs.text_search(query_search, max_results=5):
...
def diet_expertise(state: DietState) -> DietState:
    ...
    forbidden_foods = retrieve_product_info(, allowed_products=state[])
